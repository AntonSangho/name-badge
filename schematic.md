...
펠트지로 케이스를 만들 때, 배터리 선이 빠져나갈 구멍의 위치를 미리 확인해야 합니다. 아래 그림처럼 마이크로비트 왼쪽 위(배터리 커넥터가 있는 쪽)에 구멍을 뚫어주세요.

![배터리 선 구멍 위치](/img/namebadge_battery.png)
